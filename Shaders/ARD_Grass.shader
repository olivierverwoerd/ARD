Shader "ARD/ARD_Grass"
{
    Properties
    {
		...
		_TranslucentGain("Translucent Gain", Range(-1,1)) = 0.0
		[Space]
		...
		_BladeCurve("Blade Curvature Amount", Range(1, 10)) = 2
		_BendRotationRandom("Bend Rotation Random", Range(0, 1)) = 0.2
		[Header(Wind)]
		_WindDistortionMap("Wind Distortion Map", 2D) = "white" {}
		_WindStrength("Wind Strength", Float) = 0.1
		_WindDirection("Wind Direction Deg", Range(0, 360)) = 45
	...
	void geo(triangle vertexOutput IN[3], inout TriangleStream<geometryOutput> triStream)
	{
		...
		// Apply wind in direction - Olivier
		// wind direction is the directon where the wind comes from. So if direction is 0 degrees. the grass should go south
		// devision factor is 360 / 2 / pi = 57.3, to save resource on calculations we just use the answer here.
		float windx = -sin(_WindDirection / 57.3);
		float windy = -cos(_WindDirection / 57.3);
		float3 wind = normalize(float3(windx, windy, 0));
